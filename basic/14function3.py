...
: 내장함수는 외부 모듈과 달리 import 가 필요하지 않으므로 별도의
  설정없이 바로 사용할 수 있다. 
'''
print(f"{'기본 내장 함수':-^30}")
# range함수를 통해 1~10까지를 생성하여 List로 정의한다. 
...
iter() : 반복을 끝낼 값(Sentinel)을 지정하면 특정 값이 나올때
    반복을 종료하는 함수
    형식] iter(반복가능한객체, 반복을끝낼값)
'''
it = iter([1,2,3,4,5,99])
while it:
  row = next(it)
  # 99가 되면 break문을 통해 반복문 탈출
  if row == 99:
    break
  print(row, end=', ')
''' 위 while문은 더이상 출력할 항목이 없을 경우 next()에서 예외가 발생하면서
