...
'''
시나리오] 열번찍어 안넘어가는 나무 없다라는 속담을 while문으로 구현하시오
'''
treeHit = 0
while treeHit < 10 :
  treeHit = treeHit+1
...
while str:
  ...
  print(str, end=' ')
  # 인덱스 1부터 마지막까지를 슬라이싱한다. 즉 첫글자를 제거한 후 다시 대입
  str = str[1:]
...
반복문을 탈출하는 프로그램을 작성하시오. 단 break문을 사용하시오.
'''
coffee = 10
# 무한루프조건으로 while문 실행
while True:
...
'''
시나리오] 로또번호를 생성하는 프로그램을 작성하시오. 1~45사이에서
